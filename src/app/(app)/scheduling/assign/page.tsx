...
import { SingleAssignmentForm } from "@/components/scheduling/scheduling-action-forms";
...
type SchedulingAssignPageProps = {
  searchParams: Promise<Record<string, string | string[] | undefined>>;
};

export default async function SchedulingAssignPage({ searchParams }: SchedulingAssignPageProps) {
  const params = await searchParams;
  const session = await requireServerSession("/scheduling/assign");
  ...
  if (!canTenantSchedule && !canTeamSchedule) {
    return (
      <AccessDeniedPanel
        title="You do not have access to schedule assignments."
        body="Assignments are reserved for HR, tenant administrators, managers, and supervisors with scheduling authority."
      />
    );
  }
